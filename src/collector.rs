...
impl SystemCollector {
    ...
    /// Lightweight snapshot that skips `refresh_processes()` and reuses the
    /// cached process list from the last full or light refresh.
    ///
    /// Intended for dry-run mode where actions are no-ops anyway. Eliminates
    /// the ~50-100ms sysinfo process-enumeration cost from the hot path.
    pub fn collect_snapshot_no_process_refresh(&mut self) -> SystemSnapshot {
        self.sys.refresh_cpu();
        self.sys.refresh_memory();
        // No refresh_processes() — reuse cached data from last cycle.
