...
pub fn adaptive_stutter_path() -> &'static str {
    static CACHED: std::sync::OnceLock<String> = std::sync::OnceLock::new();
    CACHED.get_or_init(|| {
        std::env::var("APOLLO_ADAPTIVE_STUTTER_PATH").unwrap_or_else(|_| {
            if is_root() {
                "/var/lib/apollo/adaptive_stutter_envelope.json".to_string()
            } else {
                "/tmp/apollo-adaptive_stutter_envelope.json".to_string()
            }
        })
    })
}
